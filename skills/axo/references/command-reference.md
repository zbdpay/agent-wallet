# axo Command Reference

This reference maps common user intents to exact `axo` commands and expected JSON fields.
...
```bash
axo receive <amount_sats> [description]
```
...
```bash
axo receive --static [amount_sats] [description]
```
...
Paylinks are hosted payment pages managed via the `axobot` service. All paylink commands use `AXO_BASE_URL` (default `https://axo.bot`) for API calls.
...
```json
{"id":"pl_001","url":"https://axo.bot/paylinks/pl_001","status":"active","lifecycle":"active","amount_sats":250,"created_at":"2026-02-26T00:00:00.000Z","updated_at":"2026-02-26T00:10:00.000Z"}
```
...
```json
{
  "paylinks": [
    {"id":"pl_001","url":"https://axo.bot/paylinks/pl_001","status":"active","lifecycle":"active","amount_sats":250,"created_at":"2026-02-26T00:00:00.000Z","updated_at":"2026-02-26T00:10:00.000Z"},
    {"id":"pl_002","url":"https://axo.bot/paylinks/pl_002","status":"dead","lifecycle":"dead","amount_sats":500,"created_at":"2026-02-26T00:20:00.000Z","updated_at":"2026-02-26T00:25:00.000Z"}
  ]
}
```
...
```json
{"id":"pl_001","url":"https://axo.bot/paylinks/pl_001","status":"dead","lifecycle":"dead"}
```
...
```bash
axo fetch <url> [--method <http_method>] [--data <json>] [--max-sats <amount>]
```
...
Onchain payout commands call `AXO_BASE_URL` (default `https://axo.bot`). All commands use both `apikey` and `x-api-key` headers.
...
Side effect: appends a record to `~/.axo-wallet/payments.json` with `source: "onchain"`, `onchain_network: "bitcoin"`, `onchain_address`, and `onchain_payout_id`.
